...
**The paper's main substantive claims hold up directionally; one headline magnitude is contingent on an undocumented formula choice.** Independently constructing the same measures from the public Census QWI API and validating against state administrative data, we reproduce the paper's directional findings across every claim we tested. The one mixed verdict concerns Claim 6 ("~225,000 education jobs were lost during the first year of the pandemic"): the pandemic-year spike is real under every formula, but the specific 225K figure only emerges from a gross-loss formula in the authors' Stata code that does not match the paper's published Equations 2-3. A formula consistent with the construct's label ("Net-Negative Job Flow", netting gains against losses) yields about half that magnitude.
...
| 6 | **~225,000 education jobs were lost during the first year of the pandemic.** | ⚠️ Mixed: direction holds, magnitude depends on a formula not described in the paper | Table A4: 224.9K total NNJF in 2019-20 (paper p3 separately says "approximately 250,000" — paper-internal inconsistency between text and Table A4) | Using the authors' code formula (`sum(FrmJbLsS)` over 4 school-year quarters): 273.8K (1.22× paper) in v2.1. Using a formula consistent with the published "Net-Negative Job Flow" label (`sum(FrmJbLsS − FrmJbGnS)` — i.e., actually netting job gains against losses): 110.6K (0.5× paper). Implementing the paper's Equation 2 literally with `EmpTotal`: ~1.1M (5× paper, dominated by EmpTotal's seasonal swing). | The pandemic spike is real and large under every formula. But the specific "~225K" magnitude only emerges from the gross-loss formula in the authors' Stata code, which subtracts no job-gains and is not the formula the published Equations 2-3 describe. A truly *net* measure — consistent with the construct name in the paper — gives roughly half. A reader following the paper's text alone cannot reproduce 225K; they will get either ~1.1M (literal Eq 2) or ~110K (literal reading of "net-negative" as netting gains against losses). The headline number is contingent on a formula that the paper does not document. Year-by-year non-pandemic totals using the authors' code formula match paper Table A4 within ±5% (2019: 176.8 vs 176.2; 2021: 145.9 vs 150.4; 2023: 161.5 vs 162.1); see the v2 sensitivity section below for the full magnitude range. |
...
**Of 12 substantive claims, 10 are confirmed quantitatively (claims 1-5, 7-11), 1 is mixed (claim 6: direction holds, headline magnitude depends on an undocumented formula choice), and 1 was not independently tested (claim 12).** No claim is contradicted by the replication.
...
The paper's substantive claims about K-12 education labor market dynamics — including the pandemic spike, racial/ethnic disparities, educational attainment gradient, and the validity of QWI as a data source — **hold up directionally under replication using only the publicly available QWI API and state administrative data**. Every directional/comparative finding survives every methodology variant we tested.

The one exception is Claim 6's specific magnitude ("~225,000 education jobs were lost during the first year of the pandemic"), which is contingent on a gross-loss formula in the authors' Stata code that does not match the paper's published Equations 2-3. A formula that takes the "Net-Negative" label literally (subtracting firm-level job gains from losses) yields roughly half that figure. The pandemic-year *direction* is robust; the headline *number* is not.
